  ...
  <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300 pb-32">
    ...
    {isLoadingEvents ? (
      <div className="flex flex-col items-center justify-center py-20">
         ...
         <p className="text-[10px] text-neutral-500 uppercase tracking-[0.2em] font-black">Menyinkronkan Agenda...</p>
      </div>
    ) : events.length > 0 ? (
      <div className="space-y-4">
        {events.map((event, index) => {
          ...
          return (
            ...
            >
              <div className="bg-[#121212] p-4 rounded-[1.75rem] relative overflow-hidden">
                <div className="flex gap-4 relative z-10 items-center">
                  <div className="flex flex-col items-center justify-center w-12 h-12 rounded-2xl bg-white/5 border border-white/10 shrink-0 group-hover:bg-[#C69C3D]/10 group-hover:border-[#C69C3D]/20 transition-colors">
      ...
      <div className="flex flex-col items-center justify-center py-16 px-6 bg-white/[0.02] rounded-[2rem] border border-white/5 border-dashed">
        <Calendar className="w-12 h-12 text-neutral-700 mb-4 opacity-50" />
        <h4 className="text-white font-bold text-sm mb-1">Agenda Kosong</h4>
        <p className="text-[10px] text-neutral-500 tracking-widest uppercase font-bold text-center leading-relaxed">Belum ada jadwal yang direncanakan untuk Anda.</p>
      </div>
    )}
  </div>
);
...
export const EventDetailContent: React.FC<EventDetailContentProps> = ({ selectedEvent, onBack }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300 pb-32 px-1">
      ...
      <div 
        className="w-full h-44 rounded-[2.5rem] relative overflow-hidden shadow-2xl border border-white/5"
        style={{ 
      ...
      >
        ...
        <div className="absolute inset-x-8 bottom-8 flex flex-col gap-3">
          <div className="flex flex-col">
            <span className="text-[9px] text-[#C69C3D] font-black uppercase tracking-[0.3em] mb-1">Detail Event</span>
            <h3 className="text-2xl font-black text-white leading-tight drop-shadow-md">{selectedEvent.title}</h3>
          </div>
          
          <div className="flex items-center gap-2">
            <div className="h-0.5 w-6 bg-[#C69C3D] rounded-full"></div>
            <span className="text-[8px] text-neutral-500 font-bold uppercase tracking-widest">Internal Agenda</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div className="p-4 rounded-[1.8rem] bg-white/5 border border-white/5">
             <p className="text-[7px] text-neutral-500 uppercase tracking-[0.2em] font-black mb-2">Waktu Mulai</p>
             <div className="flex items-center gap-2">
               <div className="w-8 h-8 rounded-xl bg-[#C69C3D]/10 flex items-center justify-center">
                  <Clock className="w-4 h-4 text-[#C69C3D]" />
               </div>
               <p className="text-xs font-black text-white">{selectedEvent.start_time || '08:00'}</p>
             </div>
          </div>
          <div className="p-4 rounded-[1.8rem] bg-white/5 border border-white/5">
             <p className="text-[7px] text-neutral-500 uppercase tracking-[0.2em] font-black mb-2">Waktu Selesai</p>
             <div className="flex items-center gap-2">
               <div className="w-8 h-8 rounded-xl bg-neutral-800 flex items-center justify-center">
                  <Clock className="w-4 h-4 text-neutral-500" />
               </div>
               <p className="text-xs font-black text-white">{selectedEvent.end_time || '17:00'}</p>
             </div>
          </div>
        </div>

        <div className="p-5 rounded-[2rem] bg-white/5 border border-white/5">
           <p className="text-[7px] text-neutral-500 uppercase tracking-[0.2em] font-black mb-3">Informasi Lokasi</p>
           <div className="flex items-start gap-4">
              ...
              <p className="text-[13px] text-neutral-300 leading-relaxed font-bold py-1">{selectedEvent.location || 'Lokasi belum ditentukan untuk agenda ini.'}</p>
           </div>
        </div>

        {selectedEvent.description && (
          <div className="p-5 rounded-[2rem] bg-white/5 border border-white/5">
             <p className="text-[7px] text-neutral-500 uppercase tracking-[0.2em] font-black mb-3">Deskripsi Agenda</p>
        ...
        <div className="p-5 rounded-[2rem] bg-white/5 border border-white/5">
           <p className="text-[7px] text-neutral-500 uppercase tracking-[0.2em] font-black mb-3">PIC / Kreator</p>
           <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-neutral-800 flex items-center justify-center shrink-0">
                 <User className="w-5 h-5 text-neutral-500" />
              </div>
              <div>
                <p className="text-xs font-black text-white">{selectedEvent.created_by_name || 'Havia Staff'}</p>
                <p className="text-[8px] text-neutral-500 uppercase font-bold tracking-widest">Penyelenggara Event</p>
              </div>
           </div>
        </div>
